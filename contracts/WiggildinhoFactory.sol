 ...
 * Wiggy if they delegate & locked FTM to a validator.
 */
contract WiggildinhoFactory is Ownable {
    ...
    uint256 public thresholdLockedStake;
    string public tokenURI;

    uint8 public constant wiggyId = 21;
    ...
    constructor(
        ...
        thresholdLockedStake = _thresholdLockedStake;
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress] || block.timestamp > endBlockTime) {
            ...
            if (!wigoGalaxy.getResidentStatus(_userAddress)) {
                ...
                uint256 lastValidatorID = sfc.lastValidatorID();
                uint256 totalLockedStake = 0;

                for (uint256 i = 1; i <= lastValidatorID; i++) {
                    uint256 lockedStake;
                    (lockedStake, , , ) = sfc.getLockupInfo(_userAddress, i);
                    if (lockedStake > 0) {
                        totalLockedStake = totalLockedStake.add(lockedStake);
                    }
                }

                if (totalLockedStake >= thresholdLockedStake) {
                    return true;
                } else {
                    return false;
                }
