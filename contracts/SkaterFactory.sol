 ...
 * Wiggy if they participated in the standard staking pool.
 */
contract SkaterFactory is Ownable {
    ...
    // WiggyMinter related
    uint256 public thresholdRewards;
    string public tokenURI;

    uint8 public constant wiggyId = 11;
    ...
    constructor(
        ...
        thresholdRewards = _thresholdRewards;
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress]) {
            ...
            if (!wigoGalaxy.getResidentStatus(_userAddress)) {
                ...
                uint256 pendingWigo = masterFarmer.pendingWigo(0, _userAddress);

                if (pendingWigo >= thresholdRewards) {
                    return true;
                } else {
                    return false;
                }
