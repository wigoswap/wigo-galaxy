...
/** @title Wiggy Gen3 Factory
 * @notice It is a contract for users to mint 'Wiggy NFTs Generation 3'.
 */
contract WiggyGen3Factory is Ownable {
    ...
    uint8 private constant numberWiggyIds = 30;

    // number of previous series (i.e. different visuals)
    uint8 private constant previousNumberWiggyIds = 27;
    ...
    /**
     * @dev Set up json extensions for wiggies 27-29
     * Assign tokenURI to look for each wiggyId in the mint function
     * Only the owner can set it.
     */
    function setWiggyJson(
        string calldata _wiggyId27Json,
        string calldata _wiggyId28Json,
        string calldata _wiggyId29Json
    ) external onlyOwner {
        wiggyIdURIs[0] = string(abi.encodePacked(ipfsHash, _wiggyId27Json));
        wiggyIdURIs[1] = string(abi.encodePacked(ipfsHash, _wiggyId28Json));
        wiggyIdURIs[2] = string(abi.encodePacked(ipfsHash, _wiggyId29Json));
    }
