 ...
 * Wiggy if they staked at least 3M WIGO automatic staking (WigoVault).
 */
contract TheBoneFactory is Ownable {
    ...
    uint256 public thresholdStaked;
    string public tokenURI;

    uint8 public constant wiggyId = 24;
    ...
    constructor(
        ...
        thresholdStaked = _thresholdStaked;
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress] || block.timestamp > endBlockTime) {
            ...
            if (!wigoGalaxy.getResidentStatus(_userAddress)) {
                ...
                uint256 wigoAtLastUserAction;
                (, , wigoAtLastUserAction, ) = wigoVault.userInfo(_userAddress);

                if (wigoAtLastUserAction >= thresholdStaked) {
                    return true;
                } else {
                    return false;
                }
