 ...
 * Wiggy if they participated in WigoFarm.
 */
contract StormHaterFactory is Ownable {
    ...
    uint8 public constant wiggyId = 8;
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress] || block.timestamp > endBlockTime) {
            ...
            if (!wigoGalaxy.getResidentStatus(_userAddress)) {
                ...
                uint256 poolLength = masterFarmer.poolLength();
                uint256 totalPendingWigo = 0;

                for (uint256 i = 1; i < poolLength; i++) {
                    uint256 pendingWigo = masterFarmer.pendingWigo(
                        i,
                        _userAddress
                    );
                    if (pendingWigo > 0) {
                        totalPendingWigo = totalPendingWigo.add(pendingWigo);
                    }
                }

                if (totalPendingWigo >= thresholdYields) {
                    return true;
                } else {
                    return false;
                }
