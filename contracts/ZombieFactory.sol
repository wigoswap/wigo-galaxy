...
contract ZombieFactory is Ownable {
    ...
    uint8 public constant wiggyId = 13;
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress]) {
            ...
            if (!wigoGalaxy.getResidentStatus(_userAddress)) {
                ...
                uint256 pendingWigoFtmRewards = masterFarmer.pendingWigo(
                    2,
                    _userAddress
                );
                uint256 pendingWigoUsdcRewards = masterFarmer.pendingWigo(
                    15,
                    _userAddress
                );

                if (
                    pendingWigoFtmRewards >= thresholdYields ||
                    pendingWigoUsdcRewards >= thresholdYields
                ) {
                    return true;
                } else {
                    return false;
