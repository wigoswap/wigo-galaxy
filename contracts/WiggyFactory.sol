...
/** @title WiggyFactory
 * @notice It is a contract for users to mint 'Wiggy NFTs'.
 */
contract WiggyFactory is Ownable {
    ...
    uint8 private constant numberWiggyIds = 7;

    // number of previous series (i.e. different visuals)
    uint8 private constant previousNumberWiggyIds = 0;
    ...
    function mintNFT(uint8 _wiggyId) external {
        ...
        string memory tokenURI = wiggyIdURIs[_wiggyId];
    ...
    /**
     * @dev Set up json extensions for wiggies 0-6
     * Assign tokenURI to look for each wiggyId in the mint function
     * Only the owner can set it.
     */
    function setWiggyJson(
        string calldata _wiggyId0Json,
        string calldata _wiggyId1Json,
        string calldata _wiggyId2Json,
        string calldata _wiggyId3Json,
        string calldata _wiggyId4Json,
        string calldata _wiggyId5Json,
        string calldata _wiggyId6Json
    ) external onlyOwner {
        wiggyIdURIs[0] = string(abi.encodePacked(ipfsHash, _wiggyId0Json));
        wiggyIdURIs[1] = string(abi.encodePacked(ipfsHash, _wiggyId1Json));
        wiggyIdURIs[2] = string(abi.encodePacked(ipfsHash, _wiggyId2Json));
        wiggyIdURIs[3] = string(abi.encodePacked(ipfsHash, _wiggyId3Json));
        wiggyIdURIs[4] = string(abi.encodePacked(ipfsHash, _wiggyId4Json));
        wiggyIdURIs[5] = string(abi.encodePacked(ipfsHash, _wiggyId5Json));
        wiggyIdURIs[6] = string(abi.encodePacked(ipfsHash, _wiggyId6Json));
    }
