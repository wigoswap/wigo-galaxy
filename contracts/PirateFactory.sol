 ...
 * Wiggy if they participated in LQDR-FTM farm.
 */
contract PirateFactory is Ownable {
    ...
    uint8 public constant wiggyId = 14;
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress] || block.timestamp > endBlockTime) {
            ...
            if (!wigoGalaxy.getResidentStatus(_userAddress)) {
                ...
                uint256 pendingLqdrFtmRewards = masterFarmer.pendingWigo(
                    14,
                    _userAddress
                );

                if (pendingLqdrFtmRewards >= thresholdYields) {
                    return true;
                } else {
                    return false;
                }
